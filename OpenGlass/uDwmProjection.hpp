...
namespace OpenGlass::uDwm
{
	...
	struct CVisual : CBaseObject
	{
		...
		void STDMETHODCALLTYPE SetInsetFromParentLeft(int inset)
		{
			DEFINE_INVOKER(CVisual::SetInsetFromParentLeft);
			return INVOKE_MEMBERFUNCTION(inset);
		}
		void STDMETHODCALLTYPE SetInsetFromParentRight(int inset)
		{
			DEFINE_INVOKER(CVisual::SetInsetFromParentRight);
			return INVOKE_MEMBERFUNCTION(inset);
		}
		void STDMETHODCALLTYPE SetInsetFromParentTop(int inset)
		{
			DEFINE_INVOKER(CVisual::SetInsetFromParentTop);
			return INVOKE_MEMBERFUNCTION(inset);
		}
		void STDMETHODCALLTYPE SetInsetFromParentBottom(int inset)
		{
			DEFINE_INVOKER(CVisual::SetInsetFromParentBottom);
			return INVOKE_MEMBERFUNCTION(inset);
		}
		void STDMETHODCALLTYPE SetInsetFromParent(MARGINS* inset)
		{
			DEFINE_INVOKER(CVisual::SetInsetFromParent);
			return INVOKE_MEMBERFUNCTION(inset);
		}
		void STDMETHODCALLTYPE SetSize(SIZE* size)
		{
			DEFINE_INVOKER(CVisual::SetSize);
			return INVOKE_MEMBERFUNCTION(size);
		}
	};
	...
	struct CText : CRenderDataVisual 
	{
		...
		void STDMETHODCALLTYPE ReleaseResources(CText* This)
		{
			DEFINE_INVOKER(CText::ReleaseResources);
			return INVOKE_MEMBERFUNCTION(This);
		}
	};
	...
	struct CTopLevelWindow : CVisual
	{
		...
		long STDMETHODCALLTYPE UpdatePinnedParts() const
		{
			DEFINE_INVOKER(CTopLevelWindow::UpdatePinnedParts);
			return INVOKE_MEMBERFUNCTION();
		}
	};
	...
	{
		static long STDMETHODCALLTYPE Create(IWICBitmap* bitmap, MARGINS* margins, CBitmapSource** ppBitmapSource)
		{
			DEFINE_INVOKER(CBitmapSource::Create);
			return INVOKE_FUNCTION(bitmap, margins, ppBitmapSource);
		}
	};
	struct CBitmapSourceArray : CBaseObject
	{
		static long STDMETHODCALLTYPE CopyAndAddRef(CBitmapSourceArray* This, const struct CBitmapSourceArray* a2)
		{
			DEFINE_INVOKER(CBitmapSourceArray::CopyAndAddRef);
			return INVOKE_FUNCTION(This, a2);
		}
		static void STDMETHODCALLTYPE ReleaseContents(CBitmapSourceArray* This, char a2)
		{
			DEFINE_INVOKER(CBitmapSourceArray::ReleaseContents);
			return INVOKE_FUNCTION(This, a2);
		}
	};
	struct CDrawImageInstruction : CRenderDataInstruction
	{
		static long STDMETHODCALLTYPE Create(CBitmapSource* pBitmapSource, RECT* rect, CDrawImageInstruction** ppDrawImageInstruction)
		{
			DEFINE_INVOKER(CDrawImageInstruction::Create);
			return INVOKE_FUNCTION(pBitmapSource, rect, ppDrawImageInstruction);
		}
	};
	struct CPopInstruction : CRenderDataInstruction
	{
		static long STDMETHODCALLTYPE Create(CPopInstruction** This)
		{
			DEFINE_INVOKER(CPopInstruction::Create);
			return INVOKE_FUNCTION(This);
		}
	};
	struct CPushTransformInstruction : CRenderDataInstruction
	{
		static long STDMETHODCALLTYPE Create(CBaseTransformProxy* proxy, CPushTransformInstruction** This)
		{
			DEFINE_INVOKER(CPushTransformInstruction::Create);
			return INVOKE_FUNCTION(proxy, This);
		}
	};
	...
	inline bool OnSymbolParsing(std::string_view functionName, std::string_view fullyUnDecoratedFunctionName, const HookHelper::OffsetStorage& offset, const PSYMBOL_INFO /*originalSymInfo*/)
	{
		if (
			...
			fullyUnDecoratedFunctionName.starts_with("CBitmapSource::") ||
			fullyUnDecoratedFunctionName.starts_with("CBitmapSourceArray::") ||
			fullyUnDecoratedFunctionName.starts_with("CDrawImageInstruction::") ||
			fullyUnDecoratedFunctionName.starts_with("CPopInstruction::") ||
			fullyUnDecoratedFunctionName.starts_with("CPushTransformInstruction::") ||
